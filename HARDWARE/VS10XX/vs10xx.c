...
// 							  
// 	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2|GPIO_Pin_3|GPIO_Pin_15;//PA2,PA3,PA15
//	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
//	GPIO_Init(GPIOF, &GPIO_InitStructure);		
////MYEND	
	SPI1_Init();
...
{	 
	...
	retry=0;
	while(VS_RD_Reg(SPI_MODE)!=0x0804)	// 软件复位,新模式  
	{
...
{			 
	...
	VS_SPK_Set(vsset.speakersw);	//控制板载喇叭状态 和IIS冲突
	VS_I2S_EN();
}

void VS_I2S_EN(void)//使能I2S	
{	  
    VS_WRAM_Write(GPIO_DDR,0xf3);
    VS_WRAM_Write(I2S_CONFIG,0x0c);
